...
export const createPatientSchema = yup.object<CreateMedicalDocumentDto>().shape({
    ...
    // BirthDate: yup.date().typeError('تاریخ تولد باید تاریخ باشد').nullable(),
    // IntroductionChannel: yup.string().nullable().optional().max(200, 'نحوه آشنایی حداکثر ۲۰۰ کاراکتر'),
    // Occupation: yup.string().nullable().optional().max(200, 'شغل حداکثر ۲۰۰ کاراکتر'),
    // PatientStatusId: yup.number().required('وضعیت بیمار الزامی است').typeError('وضعیت بیمار باید عدد باشد'),
    // MaritalStatusId: yup.number().nullable().optional().typeError('وضعیت تاهل باید عدد باشد'),
    // EducationLevelId: yup.number().nullable().optional().typeError('سطح تحصیلات باید عدد باشد'),
    // InsuranceCompanyId: yup.number().nullable().optional().typeError('شرکت بیمه باید عدد باشد'),
